...
public class HomeArticleListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof HeaderViewHolder) {
            //修改切换动画
            ((HeaderViewHolder) holder).mArticleBanner.setPageTransformer(Transformer.Default);
            //Banner条目点击事件
            ...
            ((HeaderViewHolder) holder).mArticleBanner.loadImage(new XBanner.XBannerAdapter() {
                ...
                public void loadBanner(XBanner banner, Object model, View view, int position) {
                    ...
                    //如果尺寸匹配或小于目标尺寸，则返回其原始尺寸的图像，并与ImageView.ScaleType CENTER_INSIDE结合使用，以将其居中定位在Target中。
                    //如果不是，则将其缩放，以使图像的一个尺寸等于给定尺寸，而另一个尺寸小于给定尺寸（保持图像的纵横比）。
                    requestOptions.transform(new MultiTransformation(new CenterInside(),new RoundedCorners(15)));

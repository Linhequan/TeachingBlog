...
public class VideoHtmlCssFragment extends BaseFragment implements IVideoHtmlCssViewCallback, UILoader.OnRetryClickListener, HtmlCssVideoListAdapter.OnVideoItemClickListener, OnRefreshListener, OnLoadMoreListener {
    ...
    private View createSuccessView(LayoutInflater layoutInflater, ViewGroup container) {
        ...
        //上拉和下拉刷新框架
        mRefreshLayout = mRootView.findViewById(R.id.html_css_video_refreshLayout);
        //Header
    ...
    public void onVideoListLoaded(List<Video> result, boolean noMoreData) {
        ...
        //数据回来以后，就是更新UI了
        if (mHtmlCssVideoListAdapter != null) {
    ...
    public void onLoaderMoreSuccess(List<Video> result, boolean noMoreData) {
        //进入这里说明加载数据成功了
        //数据回来以后，就是更新UI了
        if (mIsLoaderMore && mRefreshLayout != null && mHtmlCssVideoListAdapter != null) {
            mHtmlCssVideoListAdapter.setData(result);
            if (noMoreData) {
                //没有更多数据（上拉加载功能将显示没有更多数据）
    ...
    public void onRefreshSuccess(List<Video> result, boolean noMoreData) {
        //进入这里说明加载数据成功了
        //数据回来以后，就是更新UI了
        if (mIsRefresh && mRefreshLayout != null && mHtmlCssVideoListAdapter != null) {
            mHtmlCssVideoListAdapter.setData(result);
            if (noMoreData) {
    ...
    public void onRefresh(@NonNull RefreshLayout refreshLayout) {
        //下拉刷新
        if (mVideoHtmlCssPresenter != null && !mIsRefresh) {
            mVideoHtmlCssPresenter.pull2RefreshMore();
            mIsRefresh = true;
        }
    }
    ...
    public void onLoadMore(@NonNull RefreshLayout refreshLayout) {
        //上拉加载更多
        if (mVideoHtmlCssPresenter != null && !mIsLoaderMore) {
            mVideoHtmlCssPresenter.loadMore();
            mIsLoaderMore = true;
        }
    }

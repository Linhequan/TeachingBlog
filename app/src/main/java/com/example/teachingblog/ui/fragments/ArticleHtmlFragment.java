...
public class ArticleHtmlFragment extends BaseFragment implements IArticleHtmlViewCallback, HtmlArticleListAdapter.OnArticleItemClickListener, UILoader.OnRetryClickListener, OnRefreshListener, OnLoadMoreListener {
    ...
    private View createSuccessView(LayoutInflater layoutInflater, ViewGroup container) {
        ...
        //上拉和下拉刷新框架
        mRefreshLayout = mRootView.findViewById(R.id.html_article_refreshLayout);
        //Header
    ...
    public void onArticleListLoaded(List<Article> result, boolean noMoreData) {
        ...
        //数据回来以后，就是更新UI了
        if (mHtmlArticleListAdapter != null) {
    ...
    public void onLoaderMoreSuccess(List<Article> result, boolean noMoreData) {
        //进入这里说明加载数据成功了
        //数据回来以后，就是更新UI了
        if (mIsLoaderMore && mRefreshLayout != null && mHtmlArticleListAdapter != null) {
            mHtmlArticleListAdapter.setData(result);
            if (noMoreData) {
                //没有更多数据（上拉加载功能将显示没有更多数据）
    ...
    public void onRefreshSuccess(List<Article> result, boolean noMoreData) {
        //进入这里说明加载数据成功了
        //数据回来以后，就是更新UI了
        if (mIsRefresh && mRefreshLayout != null && mHtmlArticleListAdapter != null) {
            mHtmlArticleListAdapter.setData(result);
            if (noMoreData) {
    ...
    public void onRefresh(@NonNull RefreshLayout refreshLayout) {
        //下拉刷新
        if (mArticleHtmlPresenter != null && !mIsRefresh) {
            mArticleHtmlPresenter.pull2RefreshMore();
            mIsRefresh = true;
        }
    }
    ...
    public void onLoadMore(@NonNull RefreshLayout refreshLayout) {
        //上拉加载更多
        if (mArticleHtmlPresenter != null && !mIsLoaderMore) {
            mArticleHtmlPresenter.loadMore();
            mIsLoaderMore = true;
        }
    }

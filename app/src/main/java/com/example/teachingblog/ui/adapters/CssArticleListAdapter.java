...
public class CssArticleListAdapter extends RecyclerView.Adapter<CssArticleListAdapter.InnerHolder> {

    private static final String TAG = "CssArticleListAdapter";
    //文章列表的内容
    private List<Article> mDatas = new ArrayList<>();
    private OnArticleItemClickListener mArticleItemClickListener = null;

    @Override
    public CssArticleListAdapter.InnerHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_article, parent, false);
        return new InnerHolder(itemView);
    }

    @Override
    public void onBindViewHolder(CssArticleListAdapter.InnerHolder holder, final int position) {
        //设置数据

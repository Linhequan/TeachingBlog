...
import com.example.teachingblog.ui.adapters.HomeArticleListAdapter;
...
public class HomeFragment extends BaseFragment implements IHomeViewCallback, UILoader.OnRetryClickListener, HomeArticleListAdapter.OnArticleItemClickListener, OnRefreshListener, OnLoadMoreListener {
    ...
    private View createSuccessView(LayoutInflater layoutInflater, ViewGroup container) {
        ...
        //上拉和下拉刷新框架
        mRefreshLayout = mRootView.findViewById(R.id.home_refreshLayout);
        //Header
    ...
    @Override
    public void onArticleListLoaded(List<Article> result, List<Article> bannerDatas) {
        //当我们获取到推荐内容的时候，这个方法就会被调用（成功了）
        //数据回来以后，就是更新UI了
        if (mHomeArticleListAdapter != null) {
            mHomeArticleListAdapter.setData(result);
            mHomeArticleListAdapter.setBannerData(bannerDatas);
        }
    ...
    @Override
    public void onLoaderMoreSuccess(List<Article> result, List<Article> bannerDatas, boolean noMoreData) {
        //进入这里说明加载数据成功了
        //数据回来以后，就是更新UI了
        if (mIsLoaderMore && mRefreshLayout != null && mHomeArticleListAdapter != null) {
            mHomeArticleListAdapter.setData(result);
            mHomeArticleListAdapter.setBannerData(bannerDatas);
            if (noMoreData) {
    ...
    @Override
    public void onRefreshSuccess(List<Article> result, List<Article> bannerDatas, boolean noMoreData) {
        //进入这里说明加载数据成功了
        //数据回来以后，就是更新UI了
        if (mIsRefresh && mRefreshLayout != null && mHomeArticleListAdapter != null) {
            mHomeArticleListAdapter.setData(result);
            mHomeArticleListAdapter.setBannerData(bannerDatas);
            if (noMoreData) {
    ...
    public void onRefresh(@NonNull RefreshLayout refreshLayout) {
        //下拉刷新
        if (mHomePresenter != null && !mIsRefresh) {
            mHomePresenter.pull2RefreshMore();
            mIsRefresh = true;
        }
    }
    ...
    public void onLoadMore(@NonNull RefreshLayout refreshLayout) {
        //上拉加载更多
        if (mHomePresenter != null && !mIsLoaderMore) {

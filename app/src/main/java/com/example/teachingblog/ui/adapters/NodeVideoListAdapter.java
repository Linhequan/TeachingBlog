...
public class NodeVideoListAdapter extends RecyclerView.Adapter<NodeVideoListAdapter.InnerHolder> {

    private static final String TAG = "NodeVideoListAdapter";
    //视频列表的内容
    private List<Video> mDatas = new ArrayList<>();
    private OnVideoItemClickListener mVideoItemClickListener = null;

    @Override
    public NodeVideoListAdapter.InnerHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_video, parent, false);
        return new InnerHolder(itemView);
    }

    @Override
    public void onBindViewHolder(NodeVideoListAdapter.InnerHolder holder, int position) {
        //设置数据

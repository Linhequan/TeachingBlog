...
public class VueVideoListAdapter extends RecyclerView.Adapter<VueVideoListAdapter.InnerHolder> {

    private static final String TAG = "VueVideoListAdapter";
    //视频列表的内容
    private List<Video> mDatas = new ArrayList<>();
    private OnVideoItemClickListener mVideoItemClickListener = null;

    @Override
    public VueVideoListAdapter.InnerHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_video, parent, false);
        return new InnerHolder(itemView);
    }

    @Override
    public void onBindViewHolder(VueVideoListAdapter.InnerHolder holder, int position) {
        //设置数据

...
public class LifeArticleListAdapter extends RecyclerView.Adapter<LifeArticleListAdapter.InnerHolder> {

    private static final String TAG = "LifeArticleListAdapter";
    //文章列表的内容
    private List<Article> mDatas = new ArrayList<>();
    private OnArticleItemClickListener mArticleItemClickListener = null;

    @Override
    public LifeArticleListAdapter.InnerHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_article, parent, false);
        return new InnerHolder(itemView);
    }

    @Override
    public void onBindViewHolder(LifeArticleListAdapter.InnerHolder holder, int position) {
        //设置数据

...
import com.example.teachingblog.interfaces.IArticleVuePresenter;
...
public class ArticleVuePresenter implements IArticleVuePresenter {

    private static final String TAG = "ArticleVuePresenter";
    private List<IArticleVueViewCallback> mCallbacks = new ArrayList<>();
    ...
    private ArticleVuePresenter() {
    }

    private static ArticleVuePresenter sInstance = null;
     ...
     * @return
     */
    public static ArticleVuePresenter getInstance() {
        if (sInstance == null) {
            synchronized (ArticleVuePresenter.class) {
                if (sInstance == null) {
                    sInstance = new ArticleVuePresenter();
                }
            }
        }
        return sInstance;
    ...
    private void doLoaded(int loadType) {
        //获取分类为vue的所有文章
        RequestCenter.getVueArticle(new DisposeDataListener() {
            @Override
    ...
    private void handlerRefreshMore(boolean noMoreData) {
        //通知UI更新
        if (mArticles != null) {
            for (IArticleVueViewCallback callback : mCallbacks) {
    ...
    @Override
    public void registerViewCallback(IArticleVueViewCallback iArticleVueViewCallback) {
        //防止重复加入
        if (mCallbacks != null && !mCallbacks.contains(iArticleVueViewCallback)) {
            mCallbacks.add(iArticleVueViewCallback);
        }
    }

    @Override
    public void unRegisterViewCallback(IArticleVueViewCallback iArticleVueViewCallback) {
        if (mCallbacks != null) {
            mCallbacks.remove(iArticleVueViewCallback);
        }
    }
}

        ...
        RequestCenter.getHtmlArticle(new DisposeDataListener() {
            ...
            public void onSuccess(Object responseObj) {
                if (responseObj != null) {
                    ...
                    //对该List进行分页
                    List<Article> articles = Utils.getArticleListPage(mCurrentPageIndex, Constants.CLASSIFICATION_COUNT, articleList);
                    ...
                    switch (loadType) {
                        case Constants.LOADER_MORE:
                            ...
                            if (articles.size() == 0) {
                                ...
                                handlerLoaderMoreResult(articles.size() < Constants.CLASSIFICATION_COUNT);
                            }
                            break;
                        case Constants.REFRESH_MORE:
                            ...
                            mArticles.clear();
                            mArticles.addAll(articles);
                            handlerRefreshMore(articles.size() < Constants.CLASSIFICATION_COUNT);
                            break;
                        case Constants.NORMAL:
                            //普通加载
                            mArticles.addAll(articles);
                            handlerArticleResult(articles.size() < Constants.CLASSIFICATION_COUNT);
                            break;
                    }
                }
            }

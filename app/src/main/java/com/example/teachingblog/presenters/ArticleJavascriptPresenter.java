...
import com.example.teachingblog.interfaces.IArticleJavascriptPresenter;
...
public class ArticleJavascriptPresenter implements IArticleJavascriptPresenter {

    private static final String TAG = "ArticleJavascriptPres";
    private List<IArticleJavascriptViewCallback> mCallbacks = new ArrayList<>();
    ...
    private ArticleJavascriptPresenter() {
    }

    private static ArticleJavascriptPresenter sInstance = null;
     ...
     * @return
     */
    public static ArticleJavascriptPresenter getInstance() {
        if (sInstance == null) {
            synchronized (ArticleJavascriptPresenter.class) {
                if (sInstance == null) {
                    sInstance = new ArticleJavascriptPresenter();
                }
            }
        }
        return sInstance;
     ...
     */
    @Override
    public void getJavascriptArticle() {
        mArticles.clear();
        this.mCurrentPageIndex = 1;
        //通知UI更新正在加载
        updateLoading();
    ...
    private void doLoaded(int loadType) {
        //获取分类为Javascript的所有文章
        RequestCenter.getJavascriptArticle(new DisposeDataListener() {
            @Override
    ...
    private void handlerRefreshMore(boolean noMoreData) {
        //通知UI更新
        if (mArticles != null) {
            for (IArticleJavascriptViewCallback callback : mCallbacks) {
    ...
    @Override
    public void registerViewCallback(IArticleJavascriptViewCallback iArticleJavascriptViewCallback) {
        //防止重复加入
        if (mCallbacks != null && !mCallbacks.contains(iArticleJavascriptViewCallback)) {
            mCallbacks.add(iArticleJavascriptViewCallback);
        }
    }

    @Override
    public void unRegisterViewCallback(IArticleJavascriptViewCallback iArticleJavascriptViewCallback) {
        if (mCallbacks != null) {
            mCallbacks.remove(iArticleJavascriptViewCallback);
        }
    }
}
